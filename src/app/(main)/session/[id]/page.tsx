...
import { Video, VideoOff, Mic, MicOff, Send, PhoneOff, User as UserIcon, Paperclip, File as FileIcon, Download } from 'lucide-react';
...
function ChatMessage({ message, author }: { message: Message, author: User | null }) {
    const { user } = useAuth();
    ...
    return (
        <div className={cn('flex items-start gap-3', isCurrentUser ? 'justify-end' : '')}>
            ...
            <div className={cn(
                ...
                {message.fileUrl && (
                    <a href={message.fileUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 mt-2 p-2 bg-background/20 rounded-md hover:bg-background/40">
...
function RemotePeerVideo({ user, stream }: { user: User | null, stream: MediaStream | null }) {
...
export default function SessionPage() {
    ...
    const { data: session, isLoading: isSessionLoading } = useSession(sessionId as string);
    ...
    const otherUserId = user?.id === session?.learnerId ? session?.mentorId : session?.learnerId;
    ...
    const setupWebRTC = useCallback(async (localStream: MediaStream, isCaller: boolean) => {
        ...
        peerConnectionRef.current.onicecandidate = event => {
            if (event.candidate && user?.id) {
                ...
                addICECandidate(sessionId as string, user.id, event.candidate.toJSON());
            }
        };
    ...
    useEffect(() => {
        ...
        const startWebRTC = async (localStream: MediaStream) => {
            unsubscribe = listenForWebRTCData(sessionId as string, user.id, otherUserId, {
                onOffer: async (offer) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = getSessionMessages(sessionId as string, (newMessages) => {
            ...
            <div className="md:col-span-2 space-y-4">
                <Card className="h-full flex flex-col">
                    ...
                    <div className="p-4 border-t flex justify-center items-center gap-4 bg-background">
                         <Button variant={isMicOn ? "outline" : "destructive"} size="icon" className="rounded-full" onClick={handleToggleMic}>
                            {isMicOn ? <Mic /> : <MicOff />}
                        </Button>
                        <Button variant={isCameraOn ? "outline" : "destructive"} size="icon" className="rounded-full" onClick={handleToggleCamera}>
                             {isCameraOn ? <Video /> : <VideoOff />}
                        </Button>
                        <Button variant="destructive" className="rounded-full px-6" onClick={handleEndCall}>
                ...
                <Card className="flex-grow flex flex-col">
                    ...
                    <ScrollArea className="flex-grow p-4" ref={scrollAreaRef}>
                        <div className="space-y-4">
                            {messages.map((msg) => (
                                <ChatMessage key={msg.id} message={msg} author={participants[msg.senderId]} />
                            ))}
                        </div>
                    </ScrollArea>
